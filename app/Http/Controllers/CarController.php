...
class CarController extends Controller
{
  ...
  public function store(Request $request)
  {
    $request->validate([
      ...
      'comprehensive_insurance' => 'nullable|file|mimes:jpeg,png,jpg,pdf|max:5120',
      'owner_id_1' => 'nullable|file|mimes:jpeg,png,jpg,pdf|max:5120',
      'owner_id_2' => 'nullable|file|mimes:jpeg,png,jpg,pdf|max:5120',
    ]);

    $data = $request->except(['image', 'or_file', 'cr_file', 'gallery_photos', 'comprehensive_insurance', 'owner_id_1', 'owner_id_2']);
    ...
    // Store comprehensive insurance if provided
    if ($request->hasFile('comprehensive_insurance')) {
      $data['comprehensive_insurance'] = $request->file('comprehensive_insurance')->store('car-docs', 'public');
    }

    // If owner ID files are uploaded, save them to the user's AffiliateDetail
    if ($request->hasFile('owner_id_1') || $request->hasFile('owner_id_2')) {
      $affData = [];
      if ($request->hasFile('owner_id_1')) {
        $affData['owner_id_1'] = $request->file('owner_id_1')->store('affiliate-docs', 'public');
      }
      if ($request->hasFile('owner_id_2')) {
        $affData['owner_id_2'] = $request->file('owner_id_2')->store('affiliate-docs', 'public');
      }
      AffiliateDetail::updateOrCreate(['user_id' => Auth::id()], $affData);
    }
  ...
  public function update(Request $request, Car $car)
  {
    ...
    $data = $request->except(['image', 'or_file', 'cr_file', 'comprehensive_insurance', 'owner_id_1', 'owner_id_2']);
